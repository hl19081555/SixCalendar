...
public class FileUtils {
    ...
    public static final String LOG_FILE = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "sixcalendar.log";
    ...
    public static boolean createFile(String fileName) {
        ...
        Log.d(TAG, "createFile : fileName = " + fileName);
        File file = new File(fileName);
        if (file.exists()) {
            return true;
        } else {
    ...
    public static boolean copyFile(String srcPath, String destDir) {
        ...
        if (destPath.equals(srcPath)) {
            Log.d(TAG, "copyFile : 源文件路径和目标文件路径重复");
            return false;
        }
        File destFile = new File(destPath); // 目标文件
        if (destFile.exists() && destFile.isFile()) {
            Log.d(TAG, "copyFile : 该路径下已经有一个同名文件");
            return false;
        }
    ...
    public static boolean renameTo(String oldPath, String newPath) {
        if (oldPath.equals(newPath)) {
            Log.d(TAG, "renameTo : 文件重命名失败：新旧文件名绝对路径相同");
            return false;
        }
